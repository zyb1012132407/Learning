...
DLinkList sortDLinkList_L(DLinkList &L);
...
void sortDLinkList_L(DLinkList &L){
	DLinkList target;
	DLinkList tmp;
	DLinkList newL;
	initDLinkList_L(newL); 
	for(tmp = L->next,target = tmp;target != L;target = target->next){
		if(target->data < tmp->data){
			tmp = target;
		}
		tmp->next = newL->next;
		tmp->prior = newL;
		newL->next->prior = newL;
		newL->next = tmp;
	}
	printf("%d",getDLinkListLength_L(L));
	showAllOfDLinkLIst_L(L);
}
